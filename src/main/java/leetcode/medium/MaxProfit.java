...
public class MaxProfit {
     ...
     * 优化：为方便计算，现将 dp[i] 变成 dp[i+2] 整体右移，初始化 dp[0]=dp[1]=dp[2]=0，因为 prices[i] 不变，因此每次仅需要求 dp[i-j-2]-price[i-j] 的最大值即可
     * n 为 prices.length，时间复杂度：O（n），空间复杂度：O（n）
     */
    private int solution(int[] prices) {
        ...
        for (int i = 1; i < prices.length; i++) {
            maxDpPrice = Math.max(maxDpPrice, dp[i - 1] - prices[i - 1]);
            dp[i + 2] = Math.max(dp[i + 1], prices[i] + maxDpPrice);
        }
