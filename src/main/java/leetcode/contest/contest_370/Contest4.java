...
//            System.out.println(res);
    ...
    private long solution(int[] nums) {
        ...
        if (numIndexList.size() == 0) {
            return res;
        }

        // num-index
        TreeSet<Integer> numNewIndexSet = new TreeSet<>();
        for (int numIndex : numIndexList) {
            int num = nums[numIndex];
            numNewIndexSet.add(num - numIndex);
        }

        // num-index -> newIndex，离散化
        TreeMap<Integer, Integer> numNewIndexMap = new TreeMap<>();
        int count = 0;
        for (Integer numNewIndex : numNewIndexSet) {
            numNewIndexMap.put(numNewIndex, count);
            count++;
        }

        SegmentTree segmentTree = new SegmentTree(0, count);
        for (int numIndex : numIndexList) {
            int num = nums[numIndex];
            int newIndex = numNewIndexMap.get(num - numIndex);

            long curValue = segmentTree.queryForMax(0, newIndex + 1) + num;
            segmentTree.update(newIndex, newIndex + 1, curValue);
        }

        return segmentTree.queryForMax(0, count);
    }
